<template>
  <section class="relative min-h-screen flex items-center overflow-hidden pt-20" aria-label="Introduction">
    <!-- Animated background -->
    <div class="absolute inset-0 z-0" aria-hidden="true">
      ...
      <svg class="absolute inset-0 w-full h-full opacity-10" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
        ...
        <line x1="0" y1="30%" x2="100%" y2="30%" stroke="url(#line-gradient)" stroke-width="1" class="animate-pulse" />
        <line x1="0" y1="70%" x2="100%" y2="70%" stroke="url(#line-gradient)" stroke-width="1" class="animate-pulse" style="animation-delay: 0.5s;" />
      </svg>
    </div>
    ...
    <Container class="relative z-10">
      <div class="grid lg:grid-cols-2 gap-12 lg:gap-8 items-center">
        ...
        <div class="order-2 lg:order-1">
          <!-- Greeting -->
          <div class="flex items-center gap-3 mb-6 opacity-0 animate-fade-in" style="animation-delay: 0.2s; animation-fill-mode: forwards;">
            <div class="w-12 h-px bg-accent" aria-hidden="true"></div>
          ...
          <!-- Name -->
          <h1 class="text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold leading-tight mb-4">
            <span 
              v-for="(word, i) in nameWords" 
              :key="i"
              class="inline-block opacity-0 animate-slide-up"
          ...
          <!-- Title -->
          <p 
            class="text-xl md:text-2xl text-text-secondary mb-4 opacity-0 animate-fade-in"
          ...
          <!-- Description -->
          <p 
            class="text-text-secondary max-w-lg mb-8 text-base leading-relaxed opacity-0 animate-fade-in"
          ...
          <!-- CTA Buttons -->
          <div 
            class="flex flex-wrap gap-4 mb-12 opacity-0 animate-fade-in"
            style="animation-delay: 0.9s; animation-fill-mode: forwards;"
          >
            <NuxtLink
              :to="localePath('/contact')"
              class="group flex items-center gap-2 px-7 py-3.5 bg-accent hover:bg-accent-dark text-white font-medium rounded-full transition-all hover:shadow-lg hover:shadow-accent/25 hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2 focus:ring-offset-bg-primary"
            >
            ...
            <NuxtLink
              ...
              class="group flex items-center gap-2 px-7 py-3.5 bg-slate-800/50 hover:bg-slate-800 text-white font-medium rounded-full transition-all hover:-translate-y-0.5 border border-slate-700/50 focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2 focus:ring-offset-bg-primary"
            >
              <span>{{ $t('hero.cta_secondary') }}</span>
              <Icon name="lucide:briefcase" class="w-4 h-4" aria-hidden="true" />
            </NuxtLink>
          </div>
          
          <!-- Stats -->
          <div 
            class="flex flex-wrap gap-8 opacity-0 animate-fade-in"
            style="animation-delay: 1s; animation-fill-mode: forwards;"
            role="list" 
            aria-label="Statistics"
          >
            <div v-for="(stat, i) in stats" :key="i" class="flex flex-col" role="listitem">
              <span class="text-2xl md:text-3xl font-bold text-white">{{ stat.value }}</span>
              <span class="text-sm text-text-muted">{{ $t(stat.label) }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="order-1 lg:order-2 flex justify-center lg:justify-end">
          <div 
            class="relative opacity-0 animate-scale-in"
            style="animation-delay: 0.5s; animation-fill-mode: forwards;"
          >
            <!-- Decorative ring -->
            <div class="absolute inset-0 rounded-full border border-accent/20 animate-spin-slow" style="animation-duration: 30s;" aria-hidden="true"></div>
            <div class="absolute inset-4 rounded-full border border-accent/10 animate-spin-slow" style="animation-duration: 25s; animation-direction: reverse;" aria-hidden="true"></div>
            
            <!-- Main image container -->
            <div class="relative w-64 h-64 md:w-80 md:h-80 lg:w-96 lg:h-96">
              <!-- Glow effect -->
              <div class="absolute inset-0 rounded-full bg-accent/20 blur-3xl" aria-hidden="true"></div>
              ...
              <div class="relative w-full h-full rounded-full overflow-hidden border-2 border-accent/30 glow">
                ...
                />
                <!-- Overlay gradient -->
                <div class="absolute inset-0 bg-linear-to-t from-bg-primary/50 to-transparent" aria-hidden="true"></div>
              </div>
...
.animate-scale-in {
  animation: scale-in 1s cubic-bezier(0.19, 1, 0.22, 1) forwards;
}
</style>
